...
function UpdateBook() {
    ...
    const {id, name, genre_id, genre, author_id, author, price, year, copies} = location.state || {};

    const [bookId, setBookId] = useState(id || -1);
    const [bookName, setBookName] = useState(name || '');
    const [bookGenre, setBookGenre] = useState(genre_id || -1);
    const [bookAuthor, setBookAuthor] = useState(author_id || -1);
    const [bookPrice, setBookPrice] = useState(price || 0);
    const [bookReleaseYear, setBookReleaseYear] = useState(year || 0);
    const [bookCopiesSold, setBookCopiesSold] = useState(copies || 0);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            setMessage('Error: Unable to update book.')
        }
    };
        ...
        <div>
            ...
            <div className='section submit-form'>
                <form className='submit-form' onSubmit={handleSubmit}>
                    ...
                    <button className="addButton" type='submit'>Update Book</button>
                </form>
                {message && <p>{message}</p>}
            </div>
